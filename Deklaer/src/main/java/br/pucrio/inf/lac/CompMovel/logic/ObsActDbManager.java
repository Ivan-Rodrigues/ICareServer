...
public class ObsActDbManager implements IDatabaseManager {
	 ...
	 */
	 ...
	 {
		 registerNewPatient("155e11b8-e1ff-406c-9905-db066f87f47d",
				 "C:\\Users\\ivanr\\Documents\\Mestrado UFMA\\Allan\\ContextNet\\ontologies\\");
	 }
	   ...
	   public Monitoring getAllMonitoringInformation(String patientMHubID)
	   {
		   ...
		   for(OWLIndividual obsRuleInd : obsRulesInd)
		   {
			   ...
			   { // é um double event
				   ...
				   String valueTwoRestriction = sensorObsInd.getDataPropertyValues(hasValuesTwoRestrictionDataProperty,
                           obsActOntology).iterator().next().getLiteral();
	   ...
	   private ActRule getAllActRuleFromObsRule(OWLIndividual obsRuleInd)
	   {
		   ...
		   for(OWLIndividual acctuationEffectInd : acctuationEffectInds)
		   {
			  ...
			  try {
				  if (notificationIndivuduals.containsEntity(acctuationEffectInd.asOWLNamedIndividual())) { // é um notification
