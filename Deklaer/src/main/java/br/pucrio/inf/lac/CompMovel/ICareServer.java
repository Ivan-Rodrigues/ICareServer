...
import br.pucrio.inf.lac.CompMovel.model.*;
...
public class ICareServer implements UDIDataReaderListener<ApplicationObject> {
   ...
   * @param message
   */
  private void handleCepMsg(String sensor_name, UUID patientUUID, Message message)
  {
	  if(sensor_name == null)
	  {
	  ...
	  {
		  if(events.get(i).getObsRuleID().equalsIgnoreCase(sensor_name))
  ...
  {		     
		...
		{
			System.out.println("---- PERSISTE DOUBLE!"); // Not implemented yet.
		}
  ...
  private void executeNotificationEffect(ActRuleNotification actRuleEffect, Message message, Event event)
  {
		...
		if(event instanceof SingleValueEvent)
		{
            ...
            NotificationHero notificationHero = new NotificationHero(Double.parseDouble(value),sEvent.getSensorName(),
                    sEvent.getTypeOfData(), sEvent.getUnityOfMeasurement(),sEvent.getObsRuleID(), notificationStr);

            notificationStr = gson.toJson(notificationHero);
		}
  ...
  private void sendMessageToHeros(ArrayList<Person> heros, UUID gatewayId, String notificationToHero)
  {
	  	...
	  	for(int i=0; i<heros.size(); i++)
	  	{
		    ...
		    privateMessage.setNodeId(UUID.fromString("788b2b22-baa6-4c61-b1bb-01cff1f5f878"));
